...
function calculateCost(weight, distance) {
  const baseCost = 10 * distance; // For first 0.5kg
...
function calculateRouteCost(route, totalWeight) {
  ...
  // Cost for traveling between centers and picking up items
  for (let i = 0; i < route.length; i++) {
    const center = route[i];
    let travelDistance;
    
    if (i === 0) {
      // First center: start from optimal position (assume 0 initial cost)
      travelDistance = 0;
    } else {
      // Travel from previous center
      travelDistance = distances[route[i-1]][center];
    }
    
    // Add weight at this center (simplified - assume we pick up all items here)
    const centerWeight = totalWeight / route.length; // Simplified distribution
...
app.post('/calculate-delivery-cost', (req, res) => {
  try {
    ...
    // Find required centers and calculate total weight
    ...
    // Calculate cost for each route and find minimum
